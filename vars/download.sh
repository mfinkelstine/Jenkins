...
# https://github.com/mfinkelstine/Jenkins/blob/master/vars/download.sh

# Downloads and replaces existing groovy files in the local directory with the latest function/pipeline files from mfinkelstine/Jenkins if they exist upstream
#
...
#       cd vars/
#
#       curl -f https://raw.githubusercontent.com/mfinkelstine/Jenkins/master/vars/download.sh > download.sh && chmod +x download.sh
#
# Download or update any number of functions you want, given as their groovy filenames as you see in the mfinkelstine/Jenkins repo::
#
#       ./download.sh argoDeploy.groovy
#
# Or run without any args to search for every *.groovy file in the local directory under vars/ and, if available in mfinkelstine/Jenkins repo, overwrite it with the latest version from GitHub
#
#       ./download.sh
#
...
url="https://raw.githubusercontent.com/mfinkelstine/Jenkins/master/vars"
