...
//
//  [% URL  # https://github.com/mfinkelstine/Jenkins %]
//
//  [% LICENSE %]
//
//  [% MESSAGE %]
...
//  agent {
    ...
    //
    //  https://github.com/mfinkelstine/Dockerfiles/tree/master/jenkins-agent-docker
    //  https://github.com/mfinkelstine/Kubernetes-templates/blob/master/jenkins-agent.cloud-pod-DooD.yaml
    //
...
//      //
//      //    https://github.com/mfinkelstine/Kubernetes-templates/blob/master/jenkins-agent-pod.yaml
//      //
...
//  }
  ...
  environment {
    ...
    GITOPS_REPO = 'git@github.com/mfinkelstine/app-k8s'
    ...
    // if run on K8s through an ingress (see https://github.com/mfinkelstine/Kubernetes-templates/)
  ...
  stages {
    ...
    stage ('Checkout') {
      steps {
        ...
        checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: '', url: 'https://github.com/mfinkelstine/DevOps-Bash-tools']]])
      }
    }
        ...
        lock(resource: 'Git Merge Staging to Dev', inversePrecedence: true) {
          ...
          timeout(time: 5, unit: 'MINUTES') {
            ...
            sshagent (credentials: ['jenkins-ssh-key-for-github']) {
              retry(2) {
                sh 'path/to/git_merge_branch.sh staging dev'  // script in https://github.com/mfinkelstine/DevOps-Bash-tools - or see vars/gitMerge*.groovy for native shared libary
              }
            }
          }
        }
    ...
    stage('Build') {
      ...
      steps {
        ...
        timeout(time: 60, unit: 'MINUTES') {
          sh 'make'
          // or
          sh './gcp_ci_build.sh'  // script in https://github.com/mfinkelstine/DevOps-Bash-tools
  ...
  // Deploys are intentionally below Approval
      ...
      steps {
        lock(resource: "Deploy - App: $APP, Environment: $ENVIRONMENT", inversePrecedence: true) {
          ...
          timeout(time: 15, unit: 'MINUTES') {
            ...
            sh './gcp_ci_deploy_k8s.sh'  // https://github.com/mfinkelstine/DevOps-Bash-tools
            // OR
    ...
    stage('Deploy Production') {
      ...
      steps {
        ...
        sh 'path/to/gcp_ci_deploy_k8s.sh'  // https://github.com/mfinkelstine/DevOps-Bash-tools
        // OR
        argoDeploy()  // func in vars/ shared library
      }
    }
